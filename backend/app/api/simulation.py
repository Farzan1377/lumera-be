...
def list_simulations():
    ...
    try:
        ...
        if Config.AUTH_ENABLED:
            us = getattr(g, 'user_sub', None)
            if project_id:
                perr = ensure_project_owned(us, project_id)
                if perr:
                    return jsonify_error(perr)
            allowed = set(
                filter_owned_ids(us, [s.simulation_id for s in simulations], kind="simulation")
            )
            simulations = [s for s in simulations if s.simulation_id in allowed]
...
def get_simulation_history():
    ...
    try:
        ...
        if Config.AUTH_ENABLED:
            ...
            simulations = [s for s in simulations if s.simulation_id in allowed][:limit]
        else:
